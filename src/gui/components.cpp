...
QRectF CNot::boundingRect() const
{
	...
	// centre positions
	t_real ctrl_pos = m_control_bit_pos * m_raster_size;
	t_real target_pos = m_target_bit_pos * m_raster_size;
...
void CNot::paint(QPainter* painter, const QStyleOptionGraphicsItem*, QWidget*)
{
	...
	painter->translate(0., m_control_bit_pos*m_raster_size);
	painter->drawEllipse(
	...
	painter->translate(0., m_target_bit_pos*m_raster_size);
	painter->drawEllipse(
		-m_target_bit_radius/2., -m_target_bit_radius/2.,
		m_target_bit_radius, m_target_bit_radius);

	painter->drawLine(QPointF(-m_target_bit_radius/2., 0.),
		QPointF(m_target_bit_radius/2., 0.));
	painter->drawLine(QPointF(0., -m_target_bit_radius/2.),
		QPointF(0., m_target_bit_radius/2.));
	painter->setWorldTransform(trafo_orig);


	// connecting vertical line
	painter->drawLine(QPointF(0., m_control_bit_pos*m_raster_size),
		QPointF(0., m_target_bit_pos*m_raster_size));
}
